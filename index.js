...
document.body.style.backgroundImage =
  `linear-gradient(${bDirection}deg, ` +
...
Promise.all([
  fetch(
    `https://api.weather.com/v2/pws/observations/current?stationId=${env.station}` +
  ...
  fetch(
    `https://api.weather.com/v3/wx/forecast/daily/5day?geocode=${env.lattitude},` +
  ...
  .then((data) => {
    ...
    currentImg.setAttribute(
      "class",
      `fas ${wIcon(data[1].daypart[0].iconCode[0])}`
    );
...
async function servers() {
  let response = await fetch("http://ash.lan/services.php", {
    headers: {
      origin: "MyTab",
